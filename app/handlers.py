...
async def send_news(message: Message, bot: Bot, data):
    ...
    if user_step==1:
        ...
        await message.answer_photo(caption='Цитата <i>«вы получаете то, что повторяете»</i>', photo=FSInputFile("./img/photo_4.jpg"))
        ...
        await message.answer_photo(caption='Цитата <i>«Знать и не делать – все равно что не знать»</i>', photo=FSInputFile("./img/photo_5.jpg"))
    
    elif user_step==6:
        await message.answer_photo(caption='Цитата <i>«Не самосовершенствование – а самооткрытие»</i>', photo=FSInputFile("./img/photo_6.jpg"))
        ...
        await message.answer_photo(caption='Цитата <i>«Хорошие привычки делают время союзников. Плохие – врагом»</i>', photo=FSInputFile("./img/photo_7.jpg"))
...
async def start_message(message: Message, bot: Bot, state = FSMContext):
    data = await state.get_data()
    caption =  """Привет! Рада тебе🩷 Я очень ценю то, что ты доверяешь мне изменение своей жизни в лучшую сторону. Меня зовут Тая. Я арт-коуч, бизнес-наставник, музыкант и художница с собственным бизнесом, построенном на любимом творческом деле 🎨 К своим результатам я пришла сама и буду рада поделиться своими секретами, которые успешно работаю в моей жизни и жизни моих учеников. Я помогу тебе стать лучшей версией себя 🕊️"""
    await asyncio.sleep(TIME_SLEEP)
    ...
    await message.answer_video_note(video_note=video_note)
    await asyncio.sleep(TIME_SLEEP)
    await message.answer_photo(caption='Цитата <i>«Себя не находят – себя создают»</i>', photo=FSInputFile("./img/photo_1.jpg"))
    await asyncio.sleep(TIME_SLEEP)
    await message.answer('Отлично! Теперь ты знаешь меня чуть лучше😻 Я тоже хочу познакомиться с тобой и сразу после пришлю тебе файл, который поможет изменить твою жизнь к лучшему🫶🏻')
    await asyncio.sleep(TIME_SLEEP)
...
async def get_request(message: Message,bot: Bot, state = FSMContext):
    ...
    await asyncio.sleep(TIME_SLEEP)
    await message.answer_photo(caption='Цитата <i>«Единственные ограничения – те, что живут в нашем разуме»</i>', photo=FSInputFile("./img/photo_2.jpg"), reply_markup=ReplyKeyboardRemove())
    await asyncio.sleep(TIME_SLEEP)
    ...
    await asyncio.sleep(TIME_SLEEP)
    await message.answer_photo(caption='Цитата <i>«Все создается дважды. Мы можем переложить ответственность за свою жизнь на наш прошлый опыт, детство, других людей – а можем быть творцами своей жизни взяв за нее ответственность.»</i>', photo=FSInputFile("./img/photo_3.jpg"))
    await asyncio.sleep(TIME_SLEEP)
